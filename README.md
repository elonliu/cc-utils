...
```bash
# 克隆到本地插件目录
git clone <repo-url> ~/.claude/plugins/local/taskloop
```

然后在 Claude Code 的配置中启用插件。打开 `~/.claude/settings.json`，添加：
...
将 `taskloop/` 目录复制到 `~/.claude/plugins/local/taskloop/` 并启用插件。
